...
function ChatAside({
  ...
  online: SocketUser[];
  setSendDirect: Dispatch<SetStateAction<string>>;
  sendDirect: string;
}) {
  return (
    <aside className="chat__aside">
      {sendDirect !== '' ? (
        <div
          onClick={() => setSendDirect('')}
          className="aside__user default--button"
        >
          <div className="aside__user--email">Send To Everyone</div>
        </div>
      ) : (
        ''
      )}
        ...
        return (
          <div
            ...
            className={`aside__user ${
              sendDirect === email ? 'aside__user--active' : ''
            }`}
            key={`aside__user${i}`}
          >
